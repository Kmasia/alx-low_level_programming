...
void type(unsigned int type, unsigned char *e_ident);
void entry(unsigned long int entry, unsigned char *e_ident);
void closed(int elf);
 ...
 * @e_ident: n array containing the ELF class.
 */
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	reader = read(opn, header, sizeof(Elf64_Ehdr));
	if (reader == -1)
	{
		free(header);
		closed(opn);
